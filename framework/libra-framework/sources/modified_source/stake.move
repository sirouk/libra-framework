...
module diem_framework::stake {
    ...
    // NOTE(0L): keep the original error numbers.
    ...
    /// Validator status enum. We can switch to proper enum later once Move
    //supports it.
    //TODO(0L): original numbers. check if these numbers can change without node issues
    ...
    /// The Events related to each validator
    // NOTE(0L): repurposed from StakePool
    ...
    // TODO(0L): This is actually an Operator Config, but we make
    // no distinction in 0L of these.
    ...
    struct ValidatorSet has key {
        ...
        ///////// 0L ////////
        // TODO(0L): Deprecated in 0L, may break node if removed
    ...
    public fun get_owned_validator_address(owner_cap: &OwnerCapability): address {
    ...
    // TODO(0L): make this a test only function and remove from Genesis
    // #[test_only]
    ...
    // TODO(0L): deprecate this. It's only used for genesis and smoke tests.
    // instead maybe_reconfigure should be the standard.
    ...
    /// Returns instrumentation for audits: if what the validator set was, which validators qualified after failover rules, a list of validators which had missing configs and were excluded, if the new list successfully matches the actual validators after reconfiguration(actual_validator_set, qualified_on_failover, missing_configs, success)
    ...
    ValidatorConfig, ValidatorPerformance, ValidatorSet {
        ...
        let vlen = vector::length(&list_info);
        let validator_index = 0;
        while ({
            spec {
                invariant spec_validators_are_initialized(validator_set.active_validators);
    ...
    fun make_validator_set_config(list: &vector<address>): (vector<ValidatorInfo>, u128, vector<address>) acquires ValidatorConfig  {
      ...
      while ({
          spec {
              invariant spec_validators_are_initialized(next_epoch_validators);
              invariant total_voting_power < MAX_VALIDATOR_SET_SIZE;
          };
          i < vlen && i < MAX_VALIDATOR_SET_SIZE
      }) {
